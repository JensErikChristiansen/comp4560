...
namespace asgn5v1
{
	...
	public class Transformer : System.Windows.Forms.Form
	{
        ...
        private double[,] reflect(Axis axis) {
        ...
        private double[,] rotate(Axis axis, double deg, bool clockwise)
        {
            double[,] result = initArray();
		...
		private void toolBar1_ButtonClick(object sender, System.Windows.Forms.ToolBarButtonClickEventArgs e)
		{
			...
			{
                ctrans = applyTransformation(ctrans, translate(deltaX * -1, 0));
                Refresh();
			}
			if (e.Button == transrightbtn) 
			{
                ctrans = applyTransformation(ctrans, translate(deltaX, 0));
                Refresh();
			}
			if (e.Button == transupbtn)
			{
                ctrans = applyTransformation(ctrans, translate(0, deltaY * -1));
				Refresh();
			}
			
			if(e.Button == transdownbtn)
			{
                ctrans = applyTransformation(ctrans, translate(0, deltaY));
                Refresh();
			}
